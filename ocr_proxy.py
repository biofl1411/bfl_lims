...
식품제조업체가 식품검사기관에 제출하는 양식으로, 식품/축산물 제품의 품질검사를 의뢰하는 문서입니다.

★ 핵심 지시사항:
1. 손글씨(필기체) 주의: 이 문서는 대부분 손으로 작성됩니다. 각 글자의 획을 꼼꼼히 분석하세요.
2. 식품업계 맥락: 제품명은 식품 이름입니다. 예시:
   - 에그샌드위치, 에그마요사라다, 크림치즈케이크, 단호박타르트, 고구마무스케이크
   - 썬데이츠어니언맛, 모짜렐라치즈볼, 깐풍기소스, 불닭볶음면
   - 현미누룽지, 쌀과자, 떡볶이소스, 유기농두부, 매실청
   식품/음식으로 해석하세요. "어깨살"처럼 엉뚱한 단어가 되면 다시 읽으세요.
3. 식품유형 컬럼: 빵류, 과자, 기타가공품, 김치류, 소스류, 음료류, 건강기능식품, 즉석섭취식품, 즉석조리식품, 절임류, 젓갈류, 면류, 두부류 등 식품공전 분류입니다.
4. 검사항목 컬럼: 대장균군, 일반세균수, 대장균, 살모넬라, 황색포도상구균, 납, 카드뮴, 수분, 산가, 과산화물가, 타르색소, 보존료, 세균수 등 식품 분석 항목입니다.
5. 체크표시: ✓, V, ○, ●, ☑ 모두 "선택됨"입니다.
6. 날짜: "2026. 2. 11" → "2026-02-11"로 변환.
7. 확신이 낮은 손글씨는 "[?]"를 붙여주세요. 예: "에그마요사라다[?]"

아래 JSON으로 추출하세요. 값이 없으면 "". 배열이 비면 [].

{
  "testPurpose": "검사목적 (자가품질위탁검사용/품질검사(의뢰)/수입식품검사/위생검사/유통식품검사/수거검사/재검사/이의신청검사/Allergen(RT-PCR)/잔류농약(참고용)/참고용(영양성분)/참고용(소비기한설정)/참고용(기준규격외)/연구용역/항생물질(참고용) 중 택1)",
  "testField": "시험분야 (식품/축산 중 하나. 기본값 식품)",
  "companyName": "상호(업체명/영업소명)",
  ...
  "mobile": "휴대전화",
  "address": "소재지",
  "bizType": "업종 (식품제조가공업/즉석판매제조가공업/유통판매업/식품소분업/기타 중 체크된 것)",
  "reportSend": ["성적서수령방법. 체크된 것만. 우편/선발송/팩스선송부/사본메일송부 중"],
  "reportSendFax": "팩스 수령번호",
  "reportSendEmail": "메일 수령주소 (@포함)",
  "samples": [
    {
      "productName": "제품명 (식품 이름으로 해석. 손글씨 불확실하면 [?] 붙임)",
      "foodType": "식품유형 (빵류/과자/기타가공품/김치류/소스류/음료류 등)",
      "inspectionItems": "시험의뢰항목 (쉼표 구분)",
      "inspectionStandard": "단시조항/시험법",
      "manufactureNo": "접수번호 또는 제조번호",
      "manufactureDate": "제조일자 (YYYY-MM-DD)",
      "expiryDate": "소비기한 (YYYY-MM-DD)",
      "expiryDays": "소비기한 일수 (숫자만, 일수표기시만)",
      "sampleAmount": "검체량 (숫자만)",
      "sampleAmountUnit": "단위 (g/kg/ml/L)",
      "sampleCount": "검체수 (숫자, 기본 1)",
      "packageUnit": "포장단위",
      "transportStatus": "운반상태 (냉장/냉동/실온/상온)"
    }
  ],
  "remarks": "비고"
}
