...
- **중간 질문 금지**: "~할까요?", "~진행할까요?", "~연결되어 있나요?" 등 확인 질문 절대 금지. 지시받은 작업을 끝까지 실행하고 결과만 보고
...
- **한 흐름 실행**: 코드 변경 → 커밋 → push → 서버 배포를 하나의 흐름으로 끊김 없이 실행
- **취소/복원 시**: 복원만 하고 대기. "어떻게 해야 하나요?" 같은 역질문 금지
- **프리뷰 검증 금지**: 코드 변경 후 preview 서버(localhost)로 검증하지 말 것. Firebase 인증 등 제한으로 의미 없음. 검증 필요 시 실서버(192.168.0.96:8443)에서 확인
...
- **파일명(xxx.html) 형식으로 페이지를 언급하지 말 것** — 반드시 사이드바 메뉴명 또는 탭 이름으로만 설명
- **함수명(xxx()) 형식으로 기능을 언급하지 말 것** — 반드시 실제 버튼명이나 사용자가 알기 쉬운 명칭으로 설명
  - 예: `allocateReceiptNo()` → "접수번호 할당 버튼", `buildReceiptNoFromSegments()` → "세그먼트 기반 접수번호 생성"
- **Firestore 컬렉션명, 영어 기술용어를 사용자에게 노출하지 말 것** — 반드시 한국어 기능명으로 설명
  - 예: `receipts` → "접수 데이터", `formTemplates` → "기록서 양식", `mfdsTemplates` → "식약처 식품유형 템플릿"
  - 예: `fieldCodes` → "분야별 코드", `sampleNoMode` → "시료 번호 방식", `counterRef` → "접수번호 카운터"
- 사용자와의 대화, 커밋 메시지, 주석 등 모든 곳에서 동일 적용

| 사이드바 메뉴 / 탭 이름 | 비고 |
|---|---|
| 접수 관리 > 업체등록·수정 | |
| 접수 관리 > 검사목적 관리 | 검사분야, 검사목적, 품목코드, 식품유형, 시험항목 등 |
| 접수 관리 > 접수 등록 | |
| 접수 관리 > 접수 현황 | |
| 접수 관리 > 설정 | |
| 시험 결재 > 시험 진행 현황 > 결과 입력 | pageTabs |
| 시험 결재 > 시험 진행 현황 > 시험일지 | pageTabs |
| 시험 결재 > 설정 > 항목배정 | pageTabs |
| 시험 결재 > 기록서 양식 수집 | |
| 성적서 > 성적서 발급 | |
| 경영실적 | |
| 유통기한 | |
