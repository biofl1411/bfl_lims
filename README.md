...
- **커밋**: `f941cf1`
- 시험항목 코드 접두사(A~X) 기반 10개 카테고리 분류 칩 추가
  - A=이화학, B=중금속/유해물질, C=미생물, D=기생충, E=잔류농약, F=동물용의약품, G=GMO, H=이물/곰팡이, I=질병검사, X=기타/알레르기
  - 각 카테고리별 색상 구분 + 건수 배지 표시
- **500건 렌더 제한**: 전체 2,171건 일괄 렌더링 → 최대 500건씩 표시하여 성능 개선
- **코드 추가 모달**: `+ 코드 추가` 버튼 → 시험항목코드/항목명/수수료/비고 입력
  - 실시간 중복 검사: 빨간 에러(중복) / 초록 체크(사용가능) / 주황 경고(미등록 접두사)
- **분류 배지 컬럼**: 테이블에 카테고리 배지 표시
- `REF_CODE_CATEGORIES` 전역 객체: 접두사→대분류순번(majorNo) 매핑

#### 참고 사항
- Firestore `settings/refFeeMapping`에 E접두사(잔류농약) 769건 누락 상태 (기존 데이터 이슈)
- `data/mfds/test_items.json` 전체 2,940건 vs Firestore 저장 2,171건

#### [TODO] 임의 추가 코드 구분 체계
- **식약처 항목**: 기존 식약처 통합LIMS 코드 (예: `A10001`, `B20003`) — 라벨: "식약처 항목"
- **임의 추가**: 사용자가 직접 추가한 코드 — 라벨: "임의 추가"
- **임의 추가 코드 규칙**: 영문 접두사 앞에 숫자 `3`으로 시작, 자릿수는 식약처 코드와 동일
  - 예: 식약처 `A10001` (6자리) → 임의 추가 `3A0001` (6자리)
  - 예: 식약처 `B20003` (6자리) → 임의 추가 `3B0001` (6자리)
- 이 규칙으로 식약처 원본 코드와 임의 추가 코드를 명확히 구분
