...
| 날짜 | 주요 작업 |
|------|-----------|
| 2026-04-03 | **처리기한 D-day 워킹데이 통일(deadline-calc.js) + 인쇄 폼 설정 + 미생물 계산기 개선 + 항목순서 UI 교체 + 시험일지 상태관리 + 모바일 접수현황 페이지(mobileSalesStatus.html) + DIARY_RULES.md** (아래 상세) |
| 2026-04-02 | **부서별 접수현황 분리 + 처리기한 Firestore 관리 + 휴일관리 탭 이동** (아래 상세) |
| 2026-04-01 | **testDiary 표 편집 기능 완성 + 납 등 이화학 결과입력 상세폼 버그 수정** (아래 상세) |
| 2026-03-31 | **재무관리 접수현황 구성 + 세금계산서 발행관리 개선** (아래 상세) |
...
### 1. 처리기한 D-day 워킹데이 통일
- **js/deadline-calc.js** 신규: Firestore 휴일+워킹데이 설정 로드, 워킹데이 D-day 계산, 렌더링 공통 모듈
- receiptStatus/reportDispatch/salesStatus/sampleReceiptV2 → DeadlineCalc 공통 모듈 사용
- D-day: 캘린더 일수 → 오늘 포함 워킹데이 기준

### 2. 휴일관리: 사유 필드 + 월별 그룹핑
- receiptSettings.html: 휴일 등록 시 사유(reason) 입력, 월별 파란 라벨 그룹핑

### 3. 미생물 집락수 계산기
- 시험결과: 소수점 버림 정수 표시 (704.5→704), 원액: 소수점 1자리
- 유효숫자 1/2/3자리 선택 드롭다운, 유효자리 적용 행 추가

### 4. 인쇄 폼 설정
- ⚙ 인쇄 폼 설정: 동적 섹션 추출, 표시/숨김, 순서 변경, ✏️ 제목 변경, Firestore 항목코드별 저장
- 모의 계산 포함/빈 양식 선택 인쇄 (미생물+일반 계산기)
- 결과 기록란 자동 반영 (결과값+판정), 비고 삭제→4열 균등, 결과 기록란 항상 맨 아래 고정

### 5. 항목 순서 편집 UI 교체
- 3열 테이블 그리드 (인쇄 레이아웃 동일), 클릭 교환 방식 (노란 선택→초록 교환)
- 원본 인덱스 보존 버그 수정 (data-orig-idx), 선택자 버그 수정

### 6. 시험일지 상태 관리
- 검토중↔완료 토글, 완료 시 편집 잠금 (저장·순서편집 숨김), 목록 뱃지 표시
- Firestore formTemplates.diaryStatus 필드

### 7. DIARY_RULES.md
- 시험일지 계산 규칙 관리대장 최초 생성
- 금속성이물 🔒 LOCKED (검증 완료), 납·세균수 🔧 WIP

### 8. 모바일 접수 현황 (mobileSalesStatus.html) 신규
- 카드 뷰, D-day 워킹데이, 기간/상태/분야 필터, 동적 재귀 검색, 🎙️ 음성인식
- 입금 상태, 계산서 발행(여부+일자+유형), 성적서 발송(방법+일자)
- 거래처 담당자(companies 컬렉션 매핑), 계산서 담당자, 전화·메일 터치 링크
- 시료별 그룹핑 (시료 번호 뱃지), 검사항목 테이블 (미입력도 표시)
- ☰ 햄버거 메뉴 + 사이드 드로어 (매출·입금·거래처·업무일지 준비중)
- Firebase Hosting 배포 준비 완료 (외근용 외부 접속)

### 9. 기타
- formCollector.html: 컨텐츠 최대 너비 1020→1400px 확장

### 신규 파일
| 파일 | 용도 |
|------|------|
| js/deadline-calc.js | 처리기한 공통 계산 모듈 |
| mobileSalesStatus.html | 영업관리 접수 현황 모바일 전용 |
| DIARY_RULES.md | 시험일지 계산 규칙 관리대장 |

---

## 📋 2026-04-02 작업 상세 (부서별 접수현황 + 처리기한 Firestore 관리)

### 1. 영업 관리 > 접수 현황 (`salesStatus.html`)
- `receiptStatus.html` 복제, Topbar "영업 관리 › 접수 현황"
- "📝 접수등록에서 수정하기" 버튼 제거 (영업 관리에서는 직접 수정 불가)
- 사이드바 영업 관리 그룹에 접수 현황 메뉴 추가

### 2. 성적 관리 > 접수 현황 (`reportDispatch.html`)
- `receiptStatus.html` 복제, Topbar "성적 관리 › 접수 현황"
- 발송 이모지 컬럼 추가: 이메일📧 / 팩스📠 / 우편📮 / 선발송⚡ (선택됨=opacity 1, 미선택=0.18)
- 처리기한 행 색상: D-2 이하 빨강 / D-5 이하 노랑 / D-17 이하 민트 (워킹데이 12일 범위)
- 기본 정렬: 처리기한 오름차순

### 3. 자가품질위탁검사용 편집 모드 버그 수정 (`sampleReceiptV2.html`)
- **증상**: 접수현황에서 수정 진입 시 시험항목이 토스트 메시지 후 사라짐
- **원인**: `updateInspectionMode()` 2초 딜레이 코드가 편집 모드에서도 `foodTypeInput.value = ''`, `inspectionItemsEl.style.display = 'none'` 실행
- **수정**: `!window._editDocId` 조건 추가로 편집 모드 차단

### 4. 처리기한 계산 개선 (`sampleReceiptV2.html`)
- 접수일 포함 워킹데이 계산: `let added = isWorkingDay(date) ? 1 : 0`
- Firestore 휴일 반영: `loadHolidaysFromFirestore()` 추가, `firebase-ready` 후 로드
- 워킹데이 수 Firestore 관리: `_deadlineWorkingDays` 변수, `loadDeadlineWorkingDays()` 추가
  - Firestore 위치: `settings/deadlineRule` → `workingDays` 필드
  - 기본값 12일, 설정 없으면 유지

### 5. 처리기한 및 휴일관리 탭 개편 (`receiptSettings.html`)
- 탭명: "휴일 관리" → "처리기한 및 휴일관리"
- "⏱️ 처리기한 설정" 섹션 추가: 워킹데이 수 입력 → `settings/deadlineRule` 저장
- 일괄 재계산: `not-in` 쿼리 → 전체 조회 후 JS 필터링으로 변경 (compat SDK 호환)
- 일괄 재계산 시 `deadlineRule.workingDays` 사용 (하드코딩 12 제거)

### 커밋
| 해시 | 내용 |
|------|------|
| `9296167` | 처리기한 일괄 재계산 not-in 쿼리 우회 |
| `a917390` | 처리기한 워킹데이 Firestore 설정 관리 |
| `a390ec3` | 작업 일지 2026-04-02 |

---

## 📋 2026-04-01 작업 상세 (testDiary 표 편집 + 결과입력 버그)

### 1. 셀 합치기 버그 수정 (`testDiary.html`)
- 범위 선택 후 합치기 시 셀 하나 사라지는 문제
- 원인: 루프 안에서 제거 시 인덱스 밀림
- 수정: 제거 전 전체 수집 후 일괄 삭제 패턴

### 2. 표 열 너비 드래그 조정 (`testDiary.html`)
- DOM 핸들 방식(파란 블록 문제) 폐기 → 셀 경계 감지 방식 채택
- 셀 오른쪽 6px 이내 마우스 → `↔` 커서, 드래그로 열 너비 조정

### 3. 열 너비 균등 배분 버튼 (`testDiary.html`)
- 툴바에 "⇔ 균등" 버튼 추가 → 모든 열 동일 비율 배분

### 4. 납 등 이화학 결과입력 상세폼 미표시 버그 (`testResultInput.html`)
- **원인**: 시험항목명이 `"납||자가품질위탁검사용"` 형태로 저장 → `findFormTemplate("납")` 과 불일치
- **수정**: `itemName.split('||')[0].trim()` 으로 `||` 이후 제거

---

## 📋 2026-03-31 작업 상세 (재무관리 접수현황 + 세금계산서 발행관리)

### 1. paymentStats.html — 입금예측 탭 구현
- 탭바에 "입금예측" 탭 추가 (결제주기|입금패턴|**입금예측**|채권주기|...)
- `renderPredictTab()` 함수: 업체별 통계적 학습 모델 산출
  - **학습 파라미터**: μ(평균결제주기), σ(표준편차), 요일확률분포(dowHist), 입금률(payRate)
  - **신뢰도 점수 (0~100)**: CV(변동계수)×0.6 + 데이터건수점수(건당5점, 최대50)
  - **예측**: 접수일 + μ → topDow(최빈 입금요일)로 조정 → 예측 입금일
  - **입금률 100% 초과 방지**: `Math.min(100, ...)` / `Math.min(1, m.payRate)` 처리

### 2. receiptFinance.html — 재무관리 접수현황 전면 구성

#### 2-1. 결제방법 → 계산서유형 자동 매핑
- `onPayMethodChange(receiptId)`: 인라인 행 결제방법 변경 시 계산서유형 자동 매핑
  - 카드 → 카드전표 / 온라인입금 → 세금계산서 / 현금 → 현금영수증 / 나머지 → 세금계산서
  - 결제방법 "선택" 초기화 시 계산서유형도 "선택"으로 초기화
- `onExpPayMethodChange()`: 펼침 영역 결제방법 변경 시 동일 자동 매핑
- **양방향 동기화**: 인라인 행 변경 → 펼침 영역 동기화 / 펼침 영역 변경 → 인라인 행 동기화

#### 2-2. 상세 펼침(openDetailModal) 업체정보 연동 버그 수정
- 사업자/대표자/이메일 "-" 표시 문제: `_companiesMap[companyName]` 미매칭 시 bizNo로 재시도 후 접수 데이터 직접 폴백
- `_dispBizNo`, `_dispRepName`, `_dispEmail` 다단계 폴백 체인 구현

#### 2-3. 시료별 수수료 표시
- 샘플 레벨에 fee 필드 없음 → `testItems[].fee` 합산으로 해결
- `SCF_` 접두사 항목(묶음수수료)의 fee 값을 시료 수수료로 사용
- 단일 시료이면 전체 수수료를 해당 시료에 표시
- 다중 시료이면 합계 행 추가 (background: #f8f9fb, 강조)
...
- testCode 접두사 기반 그룹핑:
  ...
  - `SCF_` → 묶음수수료 제외
  - 나머지 → 항목명 그대로 표시
- 예: "잔류농약 328종, 항생물질 144종, 대장균군"

#### 2-5. 계산서유형 상세 펼침 "선택" 표시 버그 수정
- **원인**: `insertAdjacentHTML`로 HTML 삽입 시 `<option selected>` 속성이 브라우저 파싱에서 무시되는 경우 발생
- **해결**: 삽입 직후 JS로 select.value 강제 세팅
  ```javascript
...
#### 2-6. 미입력 탭 취소 버튼 추가
- 저장 버튼 옆에 회색 "취소" 버튼 추가
- `resetRow(receiptId)`: `_allReceipts` 캐시에서 마지막 저장값으로 인라인 select 되돌림
- 펼침 열려 있으면 함께 닫기

#### 2-7. 전체 탭 발행대기 수정 옆 취소 버튼 추가
- 발행대기 행의 처리 열: 수정 버튼 옆에 빨간 "취소" 버튼 추가
- `clearPayInfo(receiptId)`: Firestore에서 payMethod/invoiceType/invoiceDate 필드 삭제
  - `firebase.firestore.FieldValue.delete()` 사용
  - 로컬 `_allReceipts` 캐시도 즉시 업데이트
  - 펼침 닫기 + KPI 갱신 + 테이블 재렌더링
  - 결과: 미입력 상태로 복귀

### 3. taxInvoice.html — 세금계산서 발행 현황 파악
- 현재 구현: 내부 발행처리(`taxInvoiceIssued=true` Firestore 저장) + 더존 빌36524 형식 Excel 생성
- 홈택스 직접 API 연동 논의:
  - 홈택스 e세로 API: 공인인증서 XML 서명 필요, 서버사이드 Python 구현 필요
  - 현재 Excel(더존 빌36524 형식)로 홈택스 일괄업로드 하는 방식 사용 중
  - **홈택스 직접 연동 구현은 보류** (공인인증서 준비 + e세로 등록 필요)
- 홈택스 Excel 병합 셀 여부: 데이터 열 위치가 맞으면 더존에서 파싱 가능, 현재 미구현

### 4. 커밋 이력 (2026-03-31)
| 해시 | 내용 |
|------|------|
| e7b25d2 | 재무관리 접수현황: 상세 펼침 계산서유형 select 값 강제 세팅 버그 수정 |
| aa1f53a | 재무관리 접수현황: 결제방법 변경 시 인라인-펼침 양방향 동기화 |
| 84a683a | 재무관리 접수현황: 결제방법 선택 초기화 시 계산서유형도 초기화 |
| a2ed802 | 재무관리 접수현황: 미입력 탭 취소 버튼 추가 |
| c25162d | 재무관리 접수현황: 전체 탭 발행대기 수정 옆 취소 버튼 추가 |

---

## 🔴 다음 세션 우선 작업 (2026-04-02 기준)

### 0. 처리기한 일괄 재계산 검증
- 실서버(192.168.0.96:8443)에서 접수 관리 > 설정 > 처리기한 및 휴일관리 탭 진입
- 워킹데이 수 설정 저장 확인
- "🔄 처리기한 일괄 재계산" 클릭 → 상태 메시지에 업데이트 건수 표시되는지 확인
- 접수 현황에서 처리기한 날짜 변경 여부 확인

---

## 🔴 다음 세션 기존 작업 (2026-03-28)

### 🔐 로그인 페이지 생성 시 연계 작업 목록
로그인 시스템 구축되면 아래 기능들을 행위자(actor) 기반으로 업그레이드할 것:

#### 영업관리 — 로그인 사용자 본인 데이터만 표시 (로그인 구현 후 진행)
- **대상 메뉴**: 고객사관리, 업무일지, 업체조회, 긴급 협조
- **원칙**: 로그인한 사용자 본인이 등록/담당한 데이터만 표시
- **제외 규칙 있음** (관리자 권한 등 예외 케이스 별도 정의 필요)
- **구현 방법**: `auth.currentUser.uid` 또는 `users` 컬렉션 담당자 매칭으로 Firestore 쿼리 필터
- **보류 이유**: 로그인 시스템 미구현 → 로그인 완료 후 즉시 적용
- **입금/삭제 감사 로그**: `paymentAuditLog[]`에 action + amount + timestamp + **actor(이름)** 기록
  - 현재는 로그인 없어서 보류, 로그인 추가되면 즉시 구현
  - 표시 위치: 입금관리 모달 하단 + 접수현황 수수료/결제 탭
- **인센티브 자동 계산**: 당월 4% / 익월 2% / 익익월 1% — 담당자별 집계 (별도 메모: project_incentive.md)
- **메모/수정 이력**: 수정자 이름 자동 기록 (현재 일부 `window._currentUser` 사용 중 → 실제 이름으로 대체)
...
### 0. 🚨 PA/PB mfds_test_items 즉시 복구!
- `mfds_test_items`에서 PA 507건 + PB 299건 **잘못 삭제됨** → 즉시 재등록
- 엑셀: `카카오톡 받은 파일/참고 검사항목 - 아이랩.xlsx` (PA), `축산 참고 검사항목 - 아이랩.xlsx` (PB)
- 시험항목 탭에서 엑셀 업로드로 재등록
- `refFeeMapping.items`/`mapping`의 PA/PB 삭제는 정상 (수수료만 초기화)

### 1. 참고용매핑 수수료 업로드 매칭 개선
- PA 507건 중 94건만 매칭, PB 299건 중 171건만 매칭
- 순번 기반 PA/PB 코드 매칭 강화 (`시험분야` 헤더 기반 구분)
- 삭제 시 칩 유지 (카운트는 `mfds_test_items` 기준)

### 2. 수집기 추가 작업
- ✅ DAILY_WRITE_LIMIT 제거 완료 (Blaze 요금제)
- ✅ 수집 순서 Firestore 참조 (collector.py 수정 + 문법 검증 완료)
- 수집 순서 실제 테스트 필요
- 신규업체 알림: `prms_dt` 기준 20일 이내로 변경
- 수집 현황에 증분건수/증분일 컬럼 추가

### 3. 접수관리 > 설정 > 인허가 변경 정보 페이지 신설
- fss_changes 컬렉션 연동

### 4. 업무일지 잔여 작업
- 익일계획 > 기존거래처: 필드 비율 1:1:1:1 + 매출단위 표시
- 익일계획 > 신규영업: 업종 선택 → fss_businesses 검색 + 지역 탐색 연동
- 업무일지 저장 후 목록 표시 검증
- 통계 카드 실시간 연동
