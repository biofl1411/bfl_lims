...
## 🔵 다음 작업 목록 (2026-03-03 기준)

### 1. 참고용 영문명/체크 개선 (inspectionMgmt.html)
- 영문명 컬럼에 영문 화학명이 표시되지 않는 항목 수정
- testItemCode 없는 항목의 사용 체크박스 연동 완성
- `ENG_TO_KOR_MAP`에 누락된 항목 추가 (식약처 잔류물질정보 사이트 참고: https://residue.foodsafetykorea.go.kr/)

### 2. 참고용 칩 추가: 영양성분
- 검사관리 > 참고용 탭에 **영양성분** 칩 신규 생성
- 영양성분 하위 항목 구현 (검체유형 + 항목명 + 단위 + 수수료)
- `itemGroups` Firestore에 검사목적="영양성분" 데이터 추가

### 3. 기타 칩 정리
- 기존 칩 안의 항목 정리 (불필요 항목 제거/이동)
- 새 칩 추가 시 수수료 매핑 연동 필수
